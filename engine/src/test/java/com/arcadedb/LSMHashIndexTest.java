...
public class LSMHashIndexTest extends BaseTest {
        ...
        for (int i = 0; i < TOT; ++i) {
          ...
          for (Index index : indexes) {
            ...
            if (!value.isEmpty()) {
              try {
                index.remove(key);
                Assertions.fail("remove should be not supported");
                found++;
                total++;
    ...
    database.transaction(new Database.Transaction() {
      ...
      public void execute(Database database) {
        ...
        for (Index index : indexes) {
          ...
          try {
            index.iterator(true);
            Assertions.fail("Iterator should not be supported");
          } catch (UnsupportedOperationException e) {
            // OK
          } catch (IOException e) {
            Assertions.fail(e);
    ...
    database.transaction(new Database.Transaction() {
      ...
      public void execute(Database database) {
        ...
        for (Index index : indexes) {
          ...
          try {
            index.iterator(true, new Object[] { 10 });
            Assertions.fail("Iterator should not be supported");
          } catch (UnsupportedOperationException e) {
            // OK
          } catch (IOException e) {
    ...
    database.transaction(new Database.Transaction() {
      ...
      public void execute(Database database) {
        ...
        for (Index index : indexes) {
          ...
          try {
            index.iterator(false, new Object[] { 9 });
            Assertions.fail("Iterator should not be supported");
          } catch (UnsupportedOperationException e) {
            // OK
          } catch (IOException e) {
    ...
    database.transaction(new Database.Transaction() {
      ...
      public void execute(Database database) {
        ...
        for (Index index : indexes) {
          ...
          try {
            index.range(new Object[] { 10 }, new Object[] { 19 });
            Assertions.fail("Range should not be supported");
          } catch (UnsupportedOperationException e) {
            // OK
          } catch (IOException e) {
            Assertions.fail(e);
  ...
  protected void beginTest() {
    database.transaction(new Database.Transaction() {
      ...
      public void execute(Database database) {
        ...
        final Index[] indexes = database.getSchema().createClassIndexes(SchemaImpl.INDEX_TYPE.LSM_HASH, true, TYPE_NAME, new String[] { "id" }, 1000);
