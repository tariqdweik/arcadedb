...
public enum PAnsiCode {
  ...
  public static String format(final String message, final boolean supportsColors) {
    return (String) PVariableParser.resolveVariables(message, "$ANSI{", "}", new PVariableParserListener() {
      @Override
      public Object resolve(final String iVariable) {
        ...
        if (supportsColors) {
          ...
          for (int i = 0; i < codes.length; ++i)
            buffer.append(PAnsiCode.valueOf(codes[i].toUpperCase(Locale.ENGLISH)));

          if (pos > -1) {
            buffer.append(text);
            buffer.append(PAnsiCode.RESET);
          }
