 ...
 */
public class PMultiIterator<T> implements Iterator<T>, Iterable<T> {
  ...
  public PMultiIterator() {
    sources = new ArrayList<Object>();
  }

  public PMultiIterator(final Iterator<? extends Collection<?>> iterator) {
  ...
  public PMultiIterator<T> add(final Object iValue) {
    if (iValue != null) {
      if (sourcesIterator != null)
      ...
      while (sourcesIterator.hasNext()) {
        ...
        if (next != null) {
          ...
          if (next instanceof Iterator<?>) {
            ...
            if (arraySize > 0) {
              if (arraySize == 1)
                partialIterator = new PIterableObject<T>((T) Array.get(next, 0));
              else
                partialIterator = new PIterableObjectArray<T>(next).iterator();
              return true;
            }
          } else {
            partialIterator = new PIterableObject<T>((T) next);
            return true;
          }
        }
      }
  ...
  public PMultiIterator<T> setEmbedded(final boolean embedded) {
