...
//  private OProgressListener progressListener;
  ...
  protected void buildPatterns() {
    ...
    for (MatchExpression expr : this.matchExpressions) {
      addAliases(expr, aliasFilters, aliasUserTypes, context);
    }
  ...
  private void addAliases(MatchExpression expr, Map<String, WhereClause> aliasFilters, Map<String, String> aliasUserTypes,
    ...
    addAliases(expr.origin, aliasFilters, aliasUserTypes, context);
    for (MatchPathItem item : expr.items) {
      if (item.filter != null) {
        addAliases(item.filter, aliasFilters, aliasUserTypes, context);
      }
    }
  }
    ...
    if (alias != null) {
      ...
      if (typez != null) {
        ...
        if (previousClass == null) {
          ...
          String lower = getLowerSubclass(typez, previousClass);
          if (lower == null) {
...
//    if (class1 == null) {
//      throw new PCommandExecutionException("Type " + className1 + " not found in the schema");
//    }
//    if (class2 == null) {
//      throw new PCommandExecutionException("Type " + className2 + " not found in the schema");
//    }
