...
public class OSQLFunctionEncode extends OSQLFunctionAbstract {
  ...
  public Object execute(final PDatabase database, Object iThis, PIdentifiable iCurrentRecord, Object iCurrentResult,
    ...
    if (candidate instanceof byte[]) {
      data = (byte[]) candidate;
    } else if (candidate instanceof PRID) {
      final PRecord rec = ((PRID) candidate).getRecord();
      if (rec instanceof PBinary) {
        data = ((PBinary) rec).toByteArray();
      }
    }
    ...
    if (FORMAT_BASE64.equalsIgnoreCase(format)) {
      ...
      throw new PCommandSQLParsingException("Unknown format :" + format);
    }
  }
