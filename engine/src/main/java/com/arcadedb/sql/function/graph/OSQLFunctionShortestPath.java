...
public class OSQLFunctionShortestPath extends OSQLFunctionMathAbstract {
  ...
  public List<PRID> execute(Object iThis, final PIdentifiable iCurrentRecord, final Object iCurrentResult, final Object[] iParams,
    ...
    while (true) {
      ...
      if (Thread.interrupted())
        throw new PCommandExecutionException("The shortestPath() function has been interrupted");

      List<PRID> neighborIdentity;
   ...
   * @author Thomas Young (YJJThomasYoung@hotmail.com)
   */
  private PPair<Iterable<PVertex>, Iterable<PEdge>> getVerticesAndEdges(PVertex srcVertex, PVertex.DIRECTION direction,
      String... types) {
    if (direction == PVertex.DIRECTION.BOTH) {
      ...
      PPair<Iterable<PVertex>, Iterable<PEdge>> pair1 = getVerticesAndEdges(srcVertex, PVertex.DIRECTION.OUT, types);
      PPair<Iterable<PVertex>, Iterable<PEdge>> pair2 = getVerticesAndEdges(srcVertex, PVertex.DIRECTION.IN, types);
  ...
  protected List<PRID> walkLeft(final OSQLFunctionShortestPath.OShortestPathContext ctx) {
    ...
    if (!Boolean.TRUE.equals(ctx.edge)) {
      while (!ctx.queueLeft.isEmpty()) {
        ...
        for (PVertex neighbor : neighbors) {
          final PVertex v = (PVertex) neighbor;
          final PRID neighborIdentity = v.getIdentity();
      ...
      while (!ctx.queueLeft.isEmpty()) {
        ...
        PPair<Iterable<PVertex>, Iterable<PEdge>> neighbors;
        if (ctx.edgeType == null) {
  ...
  protected List<PRID> walkRight(final OSQLFunctionShortestPath.OShortestPathContext ctx) {
