...
public abstract class OSQLFunctionHeuristicPathFinderAbstract extends OSQLFunctionMathAbstract {
  ...
  protected              List<PVertex>   route = new LinkedList<PVertex>();
  ...
  protected abstract double getDistance(final PVertex node, final PVertex parent, final PVertex target);

  protected abstract double getHeuristicCost(final PVertex node, final PVertex parent, final PVertex target,
      OCommandContext iContext);

  protected LinkedList<PVertex> getPath() {
    final LinkedList<PVertex> path = new LinkedList<PVertex>(route);
    return path;
  }
  ...
  protected Set<PVertex> getNeighbors(final PVertex node) {
    ...
    if (node != null) {
      for (PVertex v : node.getVertices(paramDirection, paramEdgeTypeNames)) {
