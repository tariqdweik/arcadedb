...
public class OSQLFunctionAstar extends OSQLFunctionHeuristicPathFinderAbstract {
  ...
  protected Set<PVertex>          closedSet            = new HashSet<PVertex>();
  protected Map<PVertex, PVertex> cameFrom             = new HashMap<PVertex, PVertex>();

  protected Map<PVertex, Double>   gScore = new HashMap<PVertex, Double>();
  protected Map<PVertex, Double>   fScore = new HashMap<PVertex, Double>();
  protected PriorityQueue<PVertex> open   = new PriorityQueue<PVertex>(1, new Comparator<PVertex>() {

    public int compare(PVertex nodeA, PVertex nodeB) {
  ...
  private LinkedList<PVertex> internalExecute(final OCommandContext iContext, PDatabase graph) {

    PVertex start = paramSourceVertex;
    PVertex goal = paramDestinationVertex;
    ...
    while (!open.isEmpty()) {
      ...
      for (PEdge neighborEdge : getNeighborEdges(current)) {

        PVertex neighbor = getNeighbor(current, neighborEdge, graph);
  ...
  protected Set<PEdge> getNeighborEdges(final PVertex node) {
    ...
    final Set<PEdge> neighbors = new HashSet<PEdge>();
    if (node != null) {
      for (PEdge v : node.getEdges(paramDirection, paramEdgeTypeNames)) {
        final PEdge ov = v;
        if (ov != null)
          neighbors.add(ov);
      }
  ...
  private void bindAdditionalParams(Object additionalParams, OSQLFunctionAstar ctx) {
    ...
    if (additionalParams instanceof Map) {
      ...
      mapParams = ((PDocument) ((PIdentifiable) additionalParams).getRecord()).toMap();
    }
    if (mapParams != null) {
      ...
      if (mapParams.get(OSQLFunctionAstar.PARAM_DIRECTION) != null) {
        if (mapParams.get(OSQLFunctionAstar.PARAM_DIRECTION) instanceof String) {
          ctx.paramDirection = PVertex.DIRECTION
              .valueOf(stringOrDefault(mapParams.get(OSQLFunctionAstar.PARAM_DIRECTION), "OUT").toUpperCase(Locale.ENGLISH));
        } else {
          ctx.paramDirection = (PVertex.DIRECTION) mapParams.get(OSQLFunctionAstar.PARAM_DIRECTION);
        }
      }
  ...
  @Override
  protected double getDistance(final PVertex node, final PVertex parent, final PVertex target) {
    final Iterator<PEdge> edges = node.getEdges(paramDirection).iterator();
    PEdge e = null;
    while (edges.hasNext()) {
  ...
  @Override
  protected double getHeuristicCost(final PVertex node, PVertex parent, final PVertex target, OCommandContext iContext) {
    double hresult = 0.0;
