...
public class OSQLStaticReflectiveFunction extends OSQLFunctionAbstract {
  ...
  public Object execute(final PDatabase database, Object iThis, PIdentifiable iCurrentRecord, Object iCurrentResult,
    ...
    if (method == null) {
      throw new PQueryParsingException("Unable to find a function for " + name + paramsPrettyPrint.get());
    }

    try {
      return method.invoke(null, iParams);
    } catch (ReflectiveOperationException e) {
      throw new PQueryParsingException("Error executing function " + name + paramsPrettyPrint.get(), e);
    } catch (IllegalArgumentException x) {
      PLogManager.instance().error(this, "Error executing function %s", x, name);
