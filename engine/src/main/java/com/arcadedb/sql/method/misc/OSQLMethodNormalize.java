...
import com.arcadedb.utility.APatternConst;
  ...
  public Object execute(final PDatabase database, final Object iThis, PIdentifiable iCurrentRecord, OCommandContext iContext,
    ...
    if (ioResult != null) {
      final Normalizer.Form form = iParams != null && iParams.length > 0 ?
          Normalizer.Form.valueOf(PFileUtils.getStringContent(iParams[0].toString())) :
          Normalizer.Form.NFD;

      String normalized = Normalizer.normalize(ioResult.toString(), form);
      if (iParams != null && iParams.length > 1) {
        normalized = normalized.replaceAll(PFileUtils.getStringContent(iParams[0].toString()), "");
      } else {
        normalized = APatternConst.PATTERN_DIACRITICAL_MARKS.matcher(normalized).replaceAll("");
      }
      ioResult = normalized;
    }
    return ioResult;
