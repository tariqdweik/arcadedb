...
It's the main class to operate with ArcadeDB. To obtain an instance of Database, use the class `<<java-ref-database-factory.adoc#,DatabaseFactory>>`.
...
|<<_close,close()>>
|<<_commit,commit()>>
|<<_deleterecord_record,deleteRecord()>>
|<<_drop,drop()>>
|<<_getschema,getSchema()>>
|<<_isopen,isOpen()>>
|<<_iteratebucket_bucketname,iterateBucket()>>
|<<_iteratetype_classname_polymorphic,iterateType()>>
...
|<<_lookupbykey_type_properties_keys,lookupByKey()>>
|<<_lookupbyrid_rid_loadcontent,lookupByRID()>>
|<<_newdocument_typename,newDocument()>>
|<<_newedgebykeys_sourcevertextype_sourcevertexkey_sourcevertexvalue_destinationvertextype_destinationvertexkey_destinationvertexvalue_createvertexifnotexist_edgetype_bidirectional_properties,newEdgeByKeys()>>
|<<_newvertex_typename,newVertex()>>
|<<_rollback,rollback()>>
...
It returns an instance of `<<java-ref-database-async.adoc#,DatabaseAsyncExecutor>>` to execute asynchronous calls.
