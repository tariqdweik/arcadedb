...
public class PLogManager {
  ...
  public void installCustomFormatter() {
    final boolean installCustomFormatter = Boolean
        .parseBoolean(PSystemVariableResolver.resolveSystemVariables("${" + ENV_INSTALL_CUSTOM_FORMATTER + "}", "true"));
    ...
    try {
      ...
      if (log.getHandlers().length == 0) {
        ...
        h.setFormatter(new PAnsiLogFormatter());
        log.addHandler(h);
      } else {
        for (Handler h : log.getHandlers()) {
          if (h instanceof ConsoleHandler && !h.getFormatter().getClass().equals(PAnsiLogFormatter.class))
            h.setFormatter(new PAnsiLogFormatter());
        }
      }
    } catch (Exception e) {
